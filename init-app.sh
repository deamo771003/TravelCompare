...
# 檢查Users表是否存在，並修剪輸出
table_exists=$(sqlcmd -S $DB_HOST -U $DB_USERNAME -P$DB_PASSWORD -d travelCompare -Q "IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Users') SELECT 1 ELSE SELECT 0" -h -1 | tr -d '[:space:]')
